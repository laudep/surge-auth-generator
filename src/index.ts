 ...
 */
export const writeAuthFile = (content: string, directory: string | undefined): Promise<string> => new Promise((resolve, reject) => {
 ...
 */
export const generate = (credentials: Credential | Credential[], directory?: string): Promise<string> => new Promise(async (resolve, reject) => {
    ...
    if (Array.isArray(credentials)) {
        fileContents = credentials.map(credential => getCredentialString(credential)).join("\n");
    } else {
        const { username, password, caseInsensitive } = credentials;
        console.log(`${username
            ? `Set username: ${username}`
            : 'No username set.'} (${caseInsensitive ? 'not ' : ''}case sensitive)`);
