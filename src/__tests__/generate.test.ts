...
const authFile1 = path.resolve(__dirname, 'tmp/test1/AUTH');
const authFile2 = path.resolve(__dirname, 'tmp/test2/AUTH');
const authFile3 = path.resolve(__dirname, 'tmp/test3/AUTH');
...
const readFile = (filePath: string): Promise<string | Buffer> => new Promise((resolve, reject) => {
    fs.readFile(filePath, "utf8", (error: NodeJS.ErrnoException | null, data: string | Buffer) => error ? reject(error) : resolve(data));
});
...
describe('auth file with array of credentials', () => {
    ...
    test('read auth file with multiple credentials', async () => {
        readFile(authFile3).then(res => expect(res).toEqual('test:test\njohn:doe'));
    });
});
